...
def generator(npart, dim, number, operators, measure_times, mode):
    ...
    for i in range(number):
        ...
        par3 = pfs[:, i]
        par4 = ads[:, i]
        frequencies[0:, i] = Ndf.measure(npart, dim, operators, rhos[:, :, i], measure_times, mode,
                                         par1, par2, par3, par4)
    ...
    '''
    # '''
    label = np.zeros([dim ** 2 + 1 + npart*3, number], dtype=np.float32)  # allocate space
    ...
    # '''
    '''
    label = np.zeros([1 + npart * 3, number], dtype=np.float32)  # allocate space
    label[0, 0:] = wns
    label[1:1 + npart, :] = bfs
    label[1 + npart:1 + npart * 2, :] = pfs
    label[1 + npart * 2:1 + npart * 3, :] = ads
    # '''
