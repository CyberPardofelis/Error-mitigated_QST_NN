...
def operator_generator(npart, dim):
    ...
    pauli[0:, 0:, 3] = np.array([[1., 0.], [0., 0.]], dtype=complex)
...
def measure(npart, dim, operators, rhom, measure_times, mode, par1, par2, par3, par4):
    ...
    if mode == 0:
        for i in range(0, 4**npart-1):
            ...
            # rhom = noise_ad(rhom, npart, dim, mode, par4)
            # rhom = noise_bf_pf(rhom, npart, dim, mode, par2, par3)
            rhom = noise_bf(rhom, npart, dim, mode, par2)
